...
class Drone1D:
    ...
    def update(self):
        self.state = np.matmul(self.dyn_mdl, self.state) + np.array([[0.01], [0], [0]])


    def observe(self):
        sensors = {n: np.random.normal(self.state.squeeze(), sig) for n, sig in zip(self.name, self.obs_noise_sig)}

        #TODO: only consider the velocity values
        sensors = {k: v[1] for k, v in sensors.items()}
        return sensors
            

# adaptive reachable set
class AReS:
    def __init__(self, args):
